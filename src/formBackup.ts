		...
		() => {
			...
			async function saveToSessionStorage() {
				...
				allFields.forEach((data: { field: string; value: string }) => {
					...
					if (data.field === 'month') {
						const month =
							data.value.length === 1 ? `0${data.value}` : data.value;
					...
					if (data.field === 'emerContactPhone') {
						(patientData as any)['emerContactPhone'] = data.value.replace(
							/-/g,
							''
						);
						return;
			...
			function fillConfirmationScreen() {
				...
				for (let i = 1; i <= formData.orders.length; i++) {
					...
					if (
						document.getElementById(`cost-med-${i}`)!.textContent !== 'No cost'
					) {
						totalCostValue += parseFloat(
							document
								.getElementById(`cost-med-${i}`)!
								.textContent!.split('$')[1]
						);
					}
				}
			...
			const redirectToStripePayment = async (uniqueId: string) => {
				const response = await fetch(
					'https://voluble-axolotl-2e6e1c.netlify.app/.netlify/functions/create-stripe-session-ach',
					{
						method: 'POST',
			...
			function createMultiStepForm(
				...
				nextButton.addEventListener('click', () => {
					...
					if (currentStep === 2) {
						fpl = fplCalculator(
							parseFloat(
								(document.getElementById('numinhouse') as HTMLInputElement)
									.value
							),
							parseFloat(
			...
			function validateForm(
				...
				if (sexRadio.length === 0) {
					...
					document
						.getElementById('sex-radio-error')!
						.classList.remove('active');
				}
				if (citizenRadio.length === 0) {
					document
						.getElementById('citizen-radio-error')!
				...
				if (currentStep === 2) {
					...
					const fax2 = document.getElementById(
						'doc2-fax-2'
					) as HTMLInputElement;
			...
			const zipInput = document.querySelectorAll(
				...
				for (let i = 1; i < 5; i++) {
					...
					if (!isEligible) {
						...
						document
							...
							.classList.remove('hidden');
						document.querySelector('.eligability-med-name')!.textContent =
							value;
					}
				}
